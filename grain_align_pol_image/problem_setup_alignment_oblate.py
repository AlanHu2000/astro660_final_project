...
#
with open('dustkapscatmat_new_10.inp','r') as f:
...
#
# Now make a mock alignment factor model. 
#
nrang = 20
...
#
alvec = np.zeros((nx,ny*2,nz,3))
#alvec[:,:,:,2] = 1.0   # Vertical field config
rrc            = np.meshgrid(xc,yc,zc,indexing='ij')[0]
rthetac        = np.meshgrid(xc,yc,zc,indexing='ij')[1]
rphic          = np.meshgrid(xc,yc,zc,indexing='ij')[2]
eff            = 0.6
alvec[:,:,:,0] = eff*np.sin(rthetac)*np.cos(rphic)  # Circular field config
alvec[:,:,:,1] = eff*np.sin(rthetac)*np.sin(rphic)  # Circular field config
alvec[:,:,:,2] = 0.0 # Circular field config

#
# Normalize
#
#length = np.sqrt(alvec[:,:,:,0]*alvec[:,:,:,0]+alvec[:,:,:,1]*alvec[:,:,:,1]+alvec[:,:,:,2]*alvec[:,:,:,2])
#alvec[:,:,:,0] = alvec[:,:,:,0] / ( length + 1e-60 )
#alvec[:,:,:,1] = alvec[:,:,:,1] / ( length + 1e-60 )
#alvec[:,:,:,2] = alvec[:,:,:,2] / ( length + 1e-60 )
#
...
#
#radius   = 5*au
#epsal  = (np.sin( np.meshgrid(xc,yc,zc,indexing='ij')[1]*rrc)/radius)**2
#epsal[epsal>1.0] = 1.0
#alvec[:,:,:,0]  = alvec[:,:,:,0] * epsal
#alvec[:,:,:,1]  = alvec[:,:,:,1] * epsal
#alvec[:,:,:,2]  = alvec[:,:,:,2] * epsal
...
with open('dustopac.inp','w+') as f:
    ...
    f.write('new_10        Extension of name of dustkappa_***.inp file\n')
    f.write('----------------------------------------------------------------------------\n')
...
# Dust alignment data
#
with open('dustkapalignfact_new_10.inp','w+') as f:
    f.write('1\n')
    f.write('%d\n'%(dustnf))
    f.write('%d\n\n'%(nrang))
...
with open('radmc3d.inp','w+') as f:
    ...
    f.write('alignment_mode = 1\n')        #consider grain alignment scattering
